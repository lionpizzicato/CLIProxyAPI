...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const projectKeepAlive = raw['project-keepalive'] ?? raw.projectKeepAlive;
  if (isRecord(projectKeepAlive)) {
    config.projectKeepAlive = {
      enabled: normalizeBoolean(projectKeepAlive.enabled),
      url:
        typeof projectKeepAlive.url === 'string'
          ? projectKeepAlive.url
          : projectKeepAlive.url === undefined || projectKeepAlive.url === null
            ? undefined
            : String(projectKeepAlive.url)
    };

    const intervalSeconds =
      projectKeepAlive['interval-seconds'] ?? projectKeepAlive.intervalSeconds;
    if (typeof intervalSeconds === 'number' && Number.isFinite(intervalSeconds)) {
      config.projectKeepAlive.intervalSeconds = intervalSeconds;
    } else if (typeof intervalSeconds === 'string' && intervalSeconds.trim() !== '') {
      const parsed = Number(intervalSeconds);
      if (Number.isFinite(parsed)) {
        config.projectKeepAlive.intervalSeconds = parsed;
      }
    }
  }
