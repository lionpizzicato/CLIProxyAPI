...
export const configApi = {
   ...
   * 获取路由策略
   */
  async getProjectKeepAliveEnabled(): Promise<boolean> {
    const data = await apiClient.get<Record<string, unknown>>('/project-keepalive/enabled');
    return Boolean(data?.enabled ?? false);
  },

  updateProjectKeepAliveEnabled: (enabled: boolean) =>
    apiClient.put('/project-keepalive/enabled', { value: enabled }),

  async getProjectKeepAliveUrl(): Promise<string> {
    const data = await apiClient.get<Record<string, unknown>>('/project-keepalive/url');
    const value = data?.url;
    return typeof value === 'string' ? value : '';
  },

  updateProjectKeepAliveUrl: (url: string) =>
    apiClient.put('/project-keepalive/url', { value: url }),

  async getProjectKeepAliveIntervalSeconds(): Promise<number> {
    const data = await apiClient.get<Record<string, unknown>>('/project-keepalive/interval-seconds');
    const value = data?.['interval-seconds'] ?? data?.intervalSeconds ?? 0;
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : 0;
  },

  updateProjectKeepAliveIntervalSeconds: (value: number) =>
    apiClient.put('/project-keepalive/interval-seconds', { value }),
