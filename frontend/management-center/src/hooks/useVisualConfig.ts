...
function getHttpUrlError(value: string): 'invalid_http_url' | undefined {
  const trimmed = value.trim();
  if (!trimmed) return undefined;
...
export function getVisualConfigValidationErrors(
  ...
  return {
    ...
    'projectKeepAlive.url': getHttpUrlError(values.projectKeepAliveUrl),
    'projectKeepAlive.intervalSeconds': getNonNegativeIntegerError(
      values.projectKeepAliveIntervalSeconds
    ),
...
export function useVisualConfig() {
  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const projectKeepAlive = asRecord(parsed['project-keepalive']);
      ...
      const newValues: VisualConfigValues = {
        ...
        projectKeepAliveEnabled: Boolean(projectKeepAlive?.enabled),
        projectKeepAliveUrl:
          typeof projectKeepAlive?.url === 'string' ? projectKeepAlive.url : '',
        projectKeepAliveIntervalSeconds: String(projectKeepAlive?.['interval-seconds'] ?? ''),
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        if (
          docHas(doc, ['project-keepalive']) ||
          values.projectKeepAliveEnabled ||
          values.projectKeepAliveUrl.trim() ||
          values.projectKeepAliveIntervalSeconds.trim()
        ) {
          ensureMapInDoc(doc, ['project-keepalive']);
          setBooleanInDoc(doc, ['project-keepalive', 'enabled'], values.projectKeepAliveEnabled);
          setStringInDoc(doc, ['project-keepalive', 'url'], values.projectKeepAliveUrl);
          setIntFromStringInDoc(
            doc,
            ['project-keepalive', 'interval-seconds'],
            values.projectKeepAliveIntervalSeconds
          );
          deleteIfMapEmpty(doc, ['project-keepalive']);
        }
