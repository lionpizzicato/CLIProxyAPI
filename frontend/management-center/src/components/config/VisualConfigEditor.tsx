...
export function VisualConfigEditor({ values, validationErrors, disabled = false, onChange }: VisualConfigEditorProps) {
  ...
  const projectKeepAliveUrlError = getValidationMessage(
    t,
    validationErrors?.['projectKeepAlive.url']
  );
  const projectKeepAliveIntervalError = getValidationMessage(
    t,
    validationErrors?.['projectKeepAlive.intervalSeconds']
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <SectionGrid>
            ...
            <Input
              ...
              placeholder="https://github.com/lionpizzicato/Cli-Proxy-API-Management-Center"
      ...
      <ConfigSection
        title={t('config_management.visual.sections.project_keepalive.title')}
        description={t('config_management.visual.sections.project_keepalive.description')}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <ToggleRow
            title={t('config_management.visual.sections.project_keepalive.enabled')}
            description={t('config_management.visual.sections.project_keepalive.enabled_desc')}
            checked={values.projectKeepAliveEnabled}
            disabled={disabled}
            onChange={(projectKeepAliveEnabled) => onChange({ projectKeepAliveEnabled })}
          />
          <SectionGrid>
            <Input
              label={t('config_management.visual.sections.project_keepalive.url')}
              type="url"
              placeholder="https://example.com/ping"
              value={values.projectKeepAliveUrl}
              onChange={(e) => onChange({ projectKeepAliveUrl: e.target.value })}
              disabled={disabled}
              hint={t('config_management.visual.sections.project_keepalive.url_hint')}
              error={projectKeepAliveUrlError}
            />
            <Input
              label={t('config_management.visual.sections.project_keepalive.interval_seconds')}
              type="number"
              placeholder="300"
              value={values.projectKeepAliveIntervalSeconds}
              onChange={(e) => onChange({ projectKeepAliveIntervalSeconds: e.target.value })}
              disabled={disabled}
              hint={t('config_management.visual.sections.project_keepalive.interval_seconds_hint')}
              error={projectKeepAliveIntervalError}
            />
          </SectionGrid>
        </div>
      </ConfigSection>
